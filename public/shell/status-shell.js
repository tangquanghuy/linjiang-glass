...
(function () {
  ...
  const SHELL_VERSION = '2026-08-27-mobile-native-flow-v1';
  ...
  const installMobileNativeBridge = () => {
    const bridge = {
      ...
      event(type) {
        /* The document is in normal flow. Portrait size/page/overlay reports only
           need Tavern Helper to remeasure body height; no host geometry is changed. */
        if (type === 'portraitSize' || type === 'portraitPage' || type === 'overlay') {
  ...
  const bindMobileNativeRuntime = () => {
    ...
    listen(window, 'resize', queueMobileNativeHeight, { passive: true });
  };
