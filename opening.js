(()=>{
const MAP_REV='custom-home-map-v21';
...
{name:'兔子洞初音',cover:'兔子洞初音',medal:'小骑士',slot:'夜间歌杂 20:30–23:30',area:'鼓岭区 · 鼓岭石库门老公馆',tags:['唱歌','杂谈','游戏'],followers:110000,blurb:'灰蓝双马尾的兔耳主播，歌回与轻松杂谈之间总带着一点桃粉色的恶作剧气息。'},
{name:'神乐七奈',cover:'神乐七奈',medal:'辣辣军',slot:'晚间绘聊 19:30–22:30',area:'青屏山风景区 · 松涧山居小院',tags:['绘画','杂谈','游戏'],followers:180000,blurb:'银蓝长发的画师主播，清亮的声线、随手涂鸦与轻快游戏回组成她的晚间频道。'}];
const OSHI_MAX=1;
...
function normalizeCustom(raw){raw=raw||{};var start=String(raw.hoursStart||''),end=String(raw.hoursEnd||'');if((!start||!end)&&raw.hours){var m=String(raw.hours).match(/^(\d{2}:\d{2})\s*-\s*(?:[^\d]*\s*)?(\d{2}:\d{2}|24:00)$/);if(m){start=m[1];end=m[2]==='24:00'?'00:00':m[2]}}var tier=clampTier(raw.tier??raw.scale?.tier??42),homeData=cloneHome(raw.homeData),assets=raw.assets||{},parts=raw.partArt||assets.parts||{},art=raw.art||{};if(!art.src&&assets.cover)art={type:'url',src:assets.cover};return{id:raw.id||customId(),name:String(raw.name||''),handle:String(raw.handle||''),age:+raw.age||23,home:String(raw.home||(homeData&&homeData.fullName)||''),homeData:homeData,categories:Array.isArray(raw.categories)&&raw.categories.length?raw.categories.slice():['\u6742\u8c08'],tier:tier,scale:streamScale(tier),hoursStart:start,hoursEnd:end,hours:String(raw.hours||''),tone:String(raw.tone||''),seed:String(raw.seed||''),medal:String(raw.medal||''),theme:normalizeStreamerTheme(raw.theme||raw['\u4ee3\u8868\u8272']),art:{type:String(art.type||''),src:String(art.src||'')},partArt:{oral:String(parts.oral||''),chest:String(parts.chest||''),vagina:String(parts.vagina||''),anus:String(parts.anus||'')},yaml:String(raw.yaml||raw.profileYaml||''),workshopAuthor:String(raw.workshopAuthor||raw.authorName||''),workshopItemId:String(raw.workshopItemId||raw.workshop?.itemId||''),updatedAt:+raw.updatedAt||0}}
...
function currentCustomData(){var d=streamerInput();return normalizeCustom(Object.assign({},d,{id:state.activeCustomId||customId(),art:state.art,partArt:state.partArt,yaml:state.yaml,homeData:cloneHome(state.streamerHome),workshopAuthor:state.workshopAuthor,workshopItemId:state.workshopItemId}))}
function renderCustomManager(){var roster=$('#custom-roster'),archive=$('#archive-list');$('#custom-count').textContent=state.customs.length;$('#archive-count').textContent=state.archives.length;$('#archive-panel').classList.toggle('hidden',!state.archiveOpen);$('#archive-toggle').classList.toggle('on',state.archiveOpen);roster.innerHTML=state.customs.length?state.customs.map(function(c){return'<div class="custom-roster-item'+(c.id===state.activeCustomId?' on':'')+'"><button type="button" data-custom-load="'+esc(c.id)+'"><i>'+esc((c.name||c.handle||'S').slice(0,1))+'</i><span><b>'+esc(c.name||'???')+'</b><small>'+esc(c.handle||'?????')+'</small></span></button><button type="button" class="custom-export" data-custom-export="'+esc(c.id)+'" title="?? JSON">??</button><button type="button" class="custom-remove" data-custom-remove="'+esc(c.id)+'" title="??">?</button></div>'}).join(''):'<span class="custom-empty">?????????</span>';archive.innerHTML=state.archives.length?state.archives.map(function(c){return'<div class="archive-item"><button type="button" data-archive-use="'+esc(c.id)+'"><i>'+esc((c.name||c.handle||'S').slice(0,1))+'</i><span><b>'+esc(c.name)+'</b><small>'+esc(c.handle||'?????')+' ? '+esc(c.hours||'???')+'</small></span></button><button type="button" class="archive-export" data-archive-export="'+esc(c.id)+'">??</button><button type="button" class="archive-delete" data-archive-delete="'+esc(c.id)+'">??</button></div>'}).join(''):'<span class="custom-empty">???????</span>'}
function clearCustomEditor(){state.activeCustomId=null;state.categories=new Set(['\u6742\u8c08']);state.yaml='';state.art={type:'',src:''};state.partArt={oral:'',chest:'',vagina:'',anus:''};state.streamerHome=null;state.streamerTheme='';state.workshopAuthor='';state.workshopItemId='';['#streamer-name','#streamer-handle','#streamer-tone','#streamer-seed','#streamer-medal','#art-url','#part-art-oral','#part-art-chest','#part-art-vagina','#part-art-anus'].forEach(id=>$(id).value='');$('#streamer-age').value='23';$('#streamer-tier').value='42';$('#streamer-home').textContent='\u4ece\u5730\u56fe\u9009\u62e9\u4f4f\u6240';$('#profile-yaml').value='';$('#art-file').value='';updateArt('','');setStreamerHours('','');renderTags();renderThemeChoices();renderScale();syncInputs();renderProfile();$('#generate-profile').textContent='\u751f\u6210\u4eba\u8bbe\u6863\u6848';setGenerateStatus('\u8c03\u7528\u7ed3\u679c\u4e0e\u9519\u8bef\u4f1a\u663e\u793a\u5728\u8fd9\u91cc');$('#save-custom').disabled=true;renderCustomManager()}
function loadCustomEditor(raw){var c=normalizeCustom(raw);state.activeCustomId=c.id;state.categories=new Set(c.categories);state.streamerHome=c.homeData||c.home?c.homeData||{fullName:c.home,name:c.home}:null;state.streamerTheme=c.theme;state.partArt=c.partArt;state.workshopAuthor=c.workshopAuthor;state.workshopItemId=c.workshopItemId;state.yaml=c.yaml;state.art=c.art;$('#streamer-name').value=c.name;$('#streamer-handle').value=c.handle;$('#streamer-age').value=c.age;$('#streamer-tier').value=c.tier;$('#streamer-tone').value=c.tone;$('#streamer-seed').value=c.seed;$('#streamer-medal').value=c.medal;$('#streamer-home').textContent=c.home||'\u4ece\u5730\u56fe\u9009\u62e9\u4f4f\u6240';$('#art-url').value=/^https?:/i.test(c.art.src)?c.art.src:'';['oral','chest','vagina','anus'].forEach(function(k){var el=$('#part-art-'+k);if(el)el.value=c.partArt[k]||''});$('#profile-yaml').value=c.yaml;renderTags();renderThemeChoices();renderScale();setStreamerHours(c.hoursStart,c.hoursEnd);updateArt(c.art.src,c.art.type);syncInputs();renderProfile();$('#generate-profile').textContent=c.yaml?'\u91cd\u65b0\u751f\u6210':'\u751f\u6210\u4eba\u8bbe\u6863\u6848';setGenerateStatus(c.yaml?'\u5df2\u8f7d\u5165\uff0c\u53ef\u7ee7\u7eed\u4fee\u6539':'\u586b\u5199\u540e\u751f\u6210\u4eba\u8bbe\u6863\u6848');$('#save-custom').disabled=!c.yaml;renderCustomManager()}
...
function workshopPackageFromCustom(raw){var d=normalizeCustom(raw),cover=/^https?:\/\//i.test(d.art.src||'')?d.art.src:'';return{schema:'linjiang.workshop.package',schemaVersion:1,game:'linjiang',itemType:'streamer',title:d.name,summary:d.tone||'',tags:d.categories||[],coverUrl:cover,authorName:d.workshopAuthor||'',data:{name:d.name,handle:d.handle,age:d.age,home:d.home,homeData:d.homeData,categories:d.categories,tier:d.tier,hoursStart:d.hoursStart,hoursEnd:d.hoursEnd,hours:d.hours,tone:d.tone,seed:d.seed,medal:d.medal,theme:d.theme,profileYaml:d.yaml,assets:{cover:cover,parts:d.partArt||{oral:'',chest:'',vagina:'',anus:''}}}}}
...
function workshopSourceList(){var out=[],seen=new Set();function add(c,id){if(!c||!c.name||!c.yaml)return;if(seen.has(c.id||id)||seen.has(c.name))return;seen.add(c.id||id);seen.add(c.name);out.push({id:id||c.id,label:(c.name||'???')+' / '+(c.handle||'?????'),custom:c})}state.customs.forEach(function(c){add(c,'custom:'+c.id)});state.archives.forEach(function(c){add(c,'archive:'+c.id)});if(state.activeCustomId){var current=currentCustomData();if(current.name&&current.handle&&current.yaml)add(current,'current:'+current.id)}return out}
function workshopSourcePackage(id){var source=workshopSourceList().find(function(x){return x.id===id});return source?workshopPackageFromCustom(source.custom):null}
function openWorkshopPicker(){
  ...
  async function onBridgeMessage(e){
    var d=e.data;
    if(!frame||e.source!==frame.contentWindow||e.origin!==WORKSHOP_SELECT_ORIGIN||!d||d.channel!=='linjiang-workshop:bridge'||d.kind!=='request')return;
    if(d.action!=='listPublishSources'&&d.action!=='exportPublishSource')return;
    var reply={channel:'linjiang-workshop:bridge',kind:'response',id:d.id};
    try{
      if(d.action==='listPublishSources'){
        var sources=d.payload?.itemType==='streamer'?workshopSourceList().map(function(x){return{id:x.id,label:x.label}}):[];
        frame.contentWindow.postMessage(Object.assign({},reply,{ok:true,payload:{sources:sources}}),e.origin);
      }else{
        var pkg=d.payload?.itemType==='streamer'?workshopSourcePackage(d.payload.sourceId):null;
        if(!pkg)throw new Error('??????????????');
        frame.contentWindow.postMessage(Object.assign({},reply,{ok:true,payload:{package:pkg}}),e.origin);
      }
    }catch(error){frame.contentWindow.postMessage(Object.assign({},reply,{ok:false,error:error?.message||String(error)}),e.origin)}
  }
  addEventListener('message',onMessage);
  addEventListener('message',onBridgeMessage);
  box._cleanup=function(){removeEventListener('message',onMessage);removeEventListener('message',onBridgeMessage)};
...
function bindFrame(){var frame=document.querySelector('#map-iframe');try{var api=frame.contentWindow.PLATE_MAP;if(api){api.onPick(function(node){handleMapPick(node)});if(api.setOpeningTarget)api.setOpeningTarget('home');setTimeout(function(){try{if(api.fitAll)api.fitAll(0)}catch(_){}},80)}}catch(_){} }
...
function openMap(target){if(target==='player'||target==='work'){setOpeningTarget(target);go(2);return}state.mapTarget='streamer';document.querySelector('#map-title').textContent='选择自定义主播住所';document.querySelector('#map-modal').classList.add('on');var quick=document.querySelector('#quick-homes');if(quick){quick.scrollTop=0;quick.scrollLeft=0}var frame=document.querySelector('#map-iframe');if(!frame.getAttribute('src'))frame.src=MAP_URL+(MAP_URL.includes('?')?'&':'?')+'target=home';var api=modalMapApi();try{if(api){if(api.setOpeningTarget)api.setOpeningTarget('home');setTimeout(function(){try{if(api.fitAll)api.fitAll(0)}catch(_){}},80)}}catch(_){} }
...
function openingPayload(){var pname=value('#player-name'),job=state.job,commute=commuteForCurrentJob(),picked=oshiPicked(),customs=state.customs.map(normalizeCustom).filter(c=>c.name&&c.yaml),mvu={'世界信息':{'年历':'2026年4月1日','时间':{'时钟':'08:00','时段':'朝'},'位置':{'区域':state.home.fullName,'场所':'家中'},'事件提示':{}},'玩家信息':{'档案':{'姓名':pname,'性别':state.gender,'年龄':+value('#player-age')||24,'网名':value('#player-handle'),'补充设定':value('#player-note')},'体力':100,'金钱':20000,'工作':{'职业':job.place?job.name:null,'地点':job.place,'日收入':job.daily,'工作时间':job.place?job.hours:null,'今日已上班':false,'通勤':commute?{'方式':'公交优先','分钟':commute.min,'距离公里':commute.km,'费用':commute.yuan}:null},'居住地':state.home.fullName,'房产':{[state.home.name]:{'名称':state.home.name,'区域':state.home.fullName,'产权':state.home.tenure,'描述':state.home.note,'月租':state.home.rent||0,'押金':state.home.deposit||0,'售价':state.home.sale||0,'费用说明':homeCostText(state.home)}},'生活固定支出':Object.assign({'日常生活费':{'金额':60,'支付周期':'每日'}},state.home.rent?{[state.home.name+'房租']:{'金额':state.home.rent,'支付周期':'每月'}}:{}),'粉丝身份':{}}};
  ...
  customs.forEach(function(d){girls[d.name]={'羁绊':{'好感度':CUSTOM_INITIAL_FAVOR,'顺从度':0,'心情':'开朗'},'位置':{'区域':d.home||'','场所':'家中','私密度':5},'性经历':emptyExperience(),'开发度':emptyDevelopment(),'生理':{'性欲度':0,'体力':100,'尿意':20,'异常状态':{}},'直播':{'开播':false,'标题':'','热度':0,'粉丝数':d.scale.followers}};rooms[d.name]={'自定义':true,'代表色':d.theme||'','主播网名':d.handle||'','封面':d.art&&d.art.type==='url'&&/^https?:\/\//i.test(d.art.src||'')?d.art.src:'','封面类型':d.art&&d.art.type==='url'?'url':'','部位图':Object.keys(d.partArt||{}).reduce(function(out,k){if(/^https?:\/\//i.test(d.partArt[k]||''))out[{oral:'\u53e3\u8154',chest:'\u80f8\u90e8',vagina:'\u5c0f\u7a74',anus:'\u809b\u95e8'}[k]]=d.partArt[k];return out},{}),'档期':d.hours||'不固定','牌子名':d.medal||d.handle||d.name,'体量档位':d.scale.tier,'底盘热度':d.scale.base,'本场热度':0,'高能榜':[],'大航海':{'舰长':d.scale.guards,'提督':d.scale.admirals,'总督':d.scale.governors,'名单':[]}};books.push({sourceName:d.name,sourceKey:d.workshopItemId||d.name,entryName:d.workshopAuthor?'\ud83e\udde9mod \u4e3b\u64ad\u4eba\u8bbe\uff5c'+d.name+'\uff5c'+d.workshopAuthor:d.name,name:d.name,keys:[d.name,d.handle].filter(Boolean),content:String(d.yaml).trim()});uis.push({name:d.name,handle:d.handle,theme:d.theme,art:d.art,partArt:d.partArt,workshopAuthor:d.workshopAuthor})});
...
$('#archive-toggle').onclick=()=>{state.archiveOpen=!state.archiveOpen;renderCustomManager()};$('#workshop-select').onclick=openWorkshopPicker;
$('#archive-save').onclick=saveCurrentArchive;
$('#custom-roster').onclick=e=>{const remove=e.target.closest('[data-custom-remove]'),load=e.target.closest('[data-custom-load]'),exporter=e.target.closest('[data-custom-export]');if(exporter){var c=state.customs.find(x=>x.id===exporter.dataset.customExport);if(c)exportWorkshopPackage(c);return}if(remove)return removeCustom(remove.dataset.customRemove);if(load){if(state.yaml&&!saveCurrentCustom({quiet:true}))return;var c=state.customs.find(x=>x.id===load.dataset.customLoad);if(c)loadCustomEditor(c)}};
$('#archive-list').onclick=e=>{const del=e.target.closest('[data-archive-delete]'),use=e.target.closest('[data-archive-use]'),exporter=e.target.closest('[data-archive-export]');if(del)return deleteArchive(del.dataset.archiveDelete);if(use)return useArchive(use.dataset.archiveUse);if(exporter){var saved=state.archives.find(x=>x.id===exporter.dataset.archiveExport);if(saved)exportWorkshopPackage(saved)}};
...
['#streamer-name','#streamer-handle'].forEach(id=>$(id).addEventListener('input',syncInputs));['#part-art-oral','#part-art-chest','#part-art-vagina','#part-art-anus'].forEach(id=>$(id).addEventListener('input',function(){state.partArt=readPartArt()}));
