/* 外部部署/README.md 的守门人。
   ...
   断言的是**会让文档变得有害的东西**：
     ...
     · 文档里的仓库路径不存在（指向空气）
     · 外部部署/ 里有文件没被清单提到（新加了粘贴脚本却没记下来，正是文档要防的失误）
...
for (const path of [...new Set([...doc.matchAll(/`([\w\u4e00-\u9fa5./-]+\.(?:js|mjs|cjs|html|md|ts|yml|json))`/g)].map((m) => m[1]))]) {
  ...
  const candidates = path.includes('/')
    ? [path]
    : ['外部部署', 'scripts', 'scripts/lib', 'public/shell', 'tools', '.'].map((d) => join(d, path));
...
console.log('\n=== 外部部署/ 里每个文件都在清单里 ===');
for (const name of readdirSync('外部部署')) {
...
console.log('\n=== 设计性质：粘贴那几份没有重新长出逻辑 ===');
const BOOT_LIMITS = [
  ['外部部署/状态栏-引导壳.html', 12, '只该有 <style> + 两个元素 + 一句 <script src> + 兜底'],
  ['外部部署/辅助计算脚本.js', 20, '只该有礼物表 + roomMenu + 占位 api + 一句 import()'],
];
...
check(!readFileSync('外部部署/辅助计算脚本.js', 'utf8').includes('DEVELOPMENT_NOTES'),
  '引导版里没有 DEVELOPMENT_NOTES');
/* 反过来，礼物表**必须**在引导版里 —— 它是刻意的例外，理由见 README 的「什么东西必须留」。
   哪天有人「顺手清理」把它挪走，直播间卡片会静默画出空礼物栏。 */
check(readFileSync('外部部署/辅助计算脚本.js', 'utf8').includes("name: '火箭'"),
